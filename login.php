...
if(empty($username) || empty($password)) {
    $error = "<div class='alert alert-danger mt-1'> bütün alanlar doldurulmalı </div>"; 
     ...
     $sql=$db->prepare("SELECT * FROM users WHERE PASSWORD='".md5($password)."' AND username='".$username."'");
    $sql->execute();
    ...
    if($result) {
        ...
        $error = "<div class='alert alert-danger mt-1'>kulla nici adı ve ya şifre hatalıdır</div>";
