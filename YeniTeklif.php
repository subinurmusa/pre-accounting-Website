...
try {
    if (isset($_POST['submit'])) {
        ...
        $urunfiyat = isset($_POST['hiddenurunfiyat' . $index]) ? $_POST['hiddenurunfiyat' . $index] : null;
            ...
            $sql = $db->prepare("INSERT INTO `selling`(`productcode`, `costomer`, `paymentType`, `products`, `date-added`, `status`, `totalPrice`, `totalGrossPrice`, `totaliskonto`, `totalkdv`,`vadetarihi`) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?)");
        
            $sql->execute([$productcode, $musteri, $paymentType, $productsJson, $tarih, $durum, $toplamtekliftutar, $gross, $toplamiskonto, $toplamkdv,$vadetarihi]);
