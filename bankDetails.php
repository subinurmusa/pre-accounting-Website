    ...
    if (isset($_POST['submit'])) {
       ...
       //var_dump($_POST);
        if (empty($title)||empty($toplam_tutar)||empty($vade_tarihi)) {
            ...
            $sql = $db->prepare("UPDATE `bankagiderler` SET `title`=?,`issueDate`=?,`totalCost`=?,`status`=?,`dueDate`=?,`type`=? WHERE id =?");
        
            $sql->execute([$title, $duzenleme_tarihi, $toplam_tutar, $status,$vade_tarihi,"Banka Gideri",$_GET["id"]]);
