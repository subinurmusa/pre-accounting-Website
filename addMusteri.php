...
try {
    if (isset($_POST['submit'])) {
        ...
        if (empty($customername)||empty($companyname)) {
            ...
            $sql = $db->prepare("INSERT INTO `customers`(`name`, `companyName`, `vergiNumber`, `IDnumber`, `companyAddress`, `email`, `phoneNumber`,`vergidairesi`,`musterinumara`) 
                VALUES (?,?,?,?,?,?,?,?,?)");
        
            $sql->execute([$customername, $companyname, $taxnumber, $tc, $address, $email, $phoneNumber,$vergidairesi,$musterinumarasıhidden]);
                ...
                <div class="col-md-9  d-flex justify-content-around align-items-center  me-5 pe-5">
                    <div class="d-flex align-items-center justify-content-between w-100 ms-4">
                        ...
                        <i class="fa-solid fa-rectangle-list fs-5"></i>
                            ...
                            <!-- <input type="text" id="costomer" name="musteri" class="form-control w-100 ms-5 ps-5"> -->
                             <input class="form-control" type="text" id="taxnumber" name="taxnumber">
